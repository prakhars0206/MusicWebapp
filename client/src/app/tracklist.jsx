...
export default function ShowTracks({ selectedAlbum, onBack }) {
  ...
  // Fetch tracks on album selection using useEffect
  useEffect(() => {
    if (selectedAlbum) {
      fetchTracks(selectedAlbum.id);
    }
    // Cleanup logic to prevent updating unmounted components
    return () => {
  ...
  // Handle case when selectedAlbum is not ready yet
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-md p-6">
        <div className="flex">
          <img
            src={selectedAlbum.images?.[0]?.url || '/placeholder-image.png'} // Fallback for missing images
          ...
          <div>
            ...
            <p className="text-gray-600">
              {selectedAlbum.artists?.map((artist) => artist.name).join(', ') || 'Unknown Artists'}
            </p>
